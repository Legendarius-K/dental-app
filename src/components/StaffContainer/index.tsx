...
const StaffContainer = () => {
    return (
        ...
        <div className="flex flex-col md:flex-row w-full">
            <h2 className="text-white text-4xl lg:text-4xl m-6 font-semibold basis-1/2">Det är vi som är Tand Trygg </h2>
        ...
        <div className="flex flex-col md:flex-row w-full flex-wrap">
            {dentists.map((item, index)=> <div className="flex flex-col bg-accent items-center m-8 p-8 w-[250px] h-[250px]">
                <img src={item.pictureUrl} alt="" className="rounded-full w-[100px]"/>
                
                <h4 className="text-black text-lg">{item.name}</h4>
                <h4 className="text-black text-md">{item.title}</h4>
